...
export default function Form({id}){
    ...
    return <>
            <Tab>
                <TabHeader>
                    {Tools.getArray(activeLang).map((lang, index)=>
                        <TabList href={lang.symbol} title={Lang('public.banner_positions')} active={index==0?"true":""} >{lang.title}</TabList>
                    )}
                </TabHeader>
                <TabBody>
                    {Tools.getArray(activeLang).map((lang, index)=>
            ...
            <Box shadow={false}>
                <Input label="dimensions" refItem={[component, "dimensions"]}  required="true" />
